...
class Application extends \FastCacheFramework\Application\Application {
	/**
	 * Public constructor
	 *
	...
	public function setMessageInTransient($messages) {
		if ($existingMessages = get_transient ( 'fastcache_notices' )) {
			$messages = array_merge ( $existingMessages, $messages );
		}

		set_transient ( 'fastcache_notices', $messages, 60 * 5 );
	}
	...
	public function publishMessages($messages) {
		foreach ( $messages as $message ) {
			echo <<<HTML
<div class="fastcache-notice notice notice-{$message['type']} is-dismissible"><p>{$message['message']}</p></div>
HTML;
		}
	}
}