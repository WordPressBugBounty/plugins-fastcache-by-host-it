...
use FastCache\Core\Html\Callbacks\Cdn;
use FastCache\Core\Html\Callbacks\CombineJsCss;
use FastCache\Core\Html\Callbacks\LazyLoad;
use FastCache\Core\Css\Parser as CssParser;
use FastCache\Core\Logger;
use FastCache\Core\Url;
use FastCache\Core\Cdn as CdnCore;
...
class Processor {
	...
	protected $sRegexMarker = 'FASTCACHEREGEXMARKER';
	...
	public function processCombineJsCss() {
		...
		$this->sFileHash = serialize ( $this->oParams->getOptions () ) . FASTCACHE_VERSION . $oUri->toString ( array (
				'scheme',
				'host'
		) );
		...
		if ($this->isCombineFilesSet () || $this->oParams->get ( 'pro_http2_push_enable', '0' )) {
			...
			\FastCache\Core\GoogleFonts::isGFontPreConnected ( $this );
		}
	}
	public function isCombineFilesSet() {
		return ! Helper::isMsieLT10 () && $this->oParams->get ( 'combine_files_enable', 0 ) && ! $this->bAmpPage;
	}
	...
	public function processLazyLoad() {
		...
		if ($bLazyLoad || $this->oParams->get ( 'pro_http2_push_enable', '0' ) || $this->oParams->get ( 'pro_next_gen_images', '1' )) {
			FASTCACHE_DEBUG ? Profiler::start ( 'LazyLoadImages' ) : null;
			...
			$sHtml = '<FASTCACHE_START>' . $this->getBodyHtml ();

			preg_match ( '#(^(?:(?:<[0-9a-z]++[^>]*+>[^<]*+(?><[^0-9a-z][^<]*+)*+){0,81}))(.*+)#six', $sHtml, $aMatches );

			$sAboveFoldHtml = str_replace ( '<FASTCACHE_START>', '', $aMatches [1] );
			...
			FASTCACHE_DEBUG ? Profiler::stop ( 'LazyLoadImages', true ) : null;
		}
	}
	protected function setupLazyLoadCriteria(Parser $oParser, $bDeferred) {
		...
		\FastCache\Core\LazyLoadExtended::setupLazyLoadExtended ( $this, $oParser, $bDeferred );
	}
	public function processImageAttributes() {
		...
		if ($bImgAttributesEnabled || ($this->oParams->get ( 'lazyload_enable', '0' ) && $this->oParams->get ( 'lazyload_autosize', '0' ))) {
			FASTCACHE_DEBUG ? Profiler::start ( 'ProcessImageAttributes' ) : null;
			...
			FASTCACHE_DEBUG ? Profiler::stop ( 'ProcessImageAttributes', true ) : null;
		}
	}
	public function processCdn() {
		...
		FASTCACHE_DEBUG ? Profiler::start ( 'RunCookieLessDomain' ) : null;
		...
		FASTCACHE_DEBUG ? Profiler::stop ( 'RunCookieLessDomain', true ) : null;
	}
