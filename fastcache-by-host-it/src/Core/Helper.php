...
class Helper {
	...
	public static function minifyHtml($sHtml, $oParams) {
		FASTCACHE_DEBUG ? Profiler::start ( 'MinifyHtml' ) : null;

		if ($oParams->get ( 'combine_files_enable', 0 ) && $oParams->get ( 'html_minify', 0 )) {
			$aOptions = array ();

			if ($oParams->get ( 'css_minify', 0 )) {
				$aOptions ['cssMinifier'] = array (
						'FastCache\Minify\Css',
						'optimize'
				);
			}
			...
			FASTCACHE_DEBUG ? Profiler::stop ( 'MinifyHtml', true ) : null;
		}
	...
	public static function postAsync($url, $params, array $posts) {
		...
		if (! $fp) {
			...
			Logger::debug ( $errno . ': ' . $errstr, 'FASTCACHE_post-error' );
		} else {
			...
			fwrite ( $fp, $out );
			fclose ( $fp );
			Logger::debug ( $out, 'FASTCACHE_post' );
		}
	}
	...
	public static function findExcludes($aArray, $sString, $sType = '') {
		...
		foreach ( $aArray as $sValue ) {
			...
			if ($sValue && stripos ( htmlspecialchars_decode ( $sString ), $sValue ) !== false) {
				return true;
			}
		}
