...
use FastCacheFramework\Filesystem\File;
...
class LightImages {
	...
	private function processImageNodes(&$node, $srcSet = false, $srcSetIteration = null, $qualitySrcSet = null, $resizeFactorSrcSet = null, $originalSource = null) {
		...
		$cache_path = FASTCACHE_CACHE_DIR . 'images';
		$cache_path_http = rtrim(Uri::root(true), '/') . "/wp-content/cache/fastcache/images";
		...
		if(strpos($src, 'data:image/svg+xml;bas' . 'e64') !== false && $node->hasAttribute('data-fastcache-lazyload')) {
		...
		$fastCacheBrowser = Browser::getInstance()->getBrowser ();
		$excludeJSpeedBrowserSafari = $this->params->get('exclude_light_images_safari', 1) && $fastCacheBrowser == 'Safari';
		if($this->params->get('convert_all_images_to_webp', 0) && function_exists('imagewebp') && $fastCacheBrowser != 'IE' && !$excludeJSpeedBrowserSafari) {
			$new_ext = 'webp';
		}

		// Override webp if avif is supported and enabled
		if($this->params->get('convert_all_images_to_avif', 0) && version_compare(PHP_VERSION, '8.1', '>=') && function_exists('imageavif') && $fastCacheBrowser != 'IE' && $fastCacheBrowser != 'Edge' && $fastCacheBrowser != 'Opera Mini' && !$excludeJSpeedBrowserSafari) {
			$new_ext = 'avif';
		}
		...
		if ($createSrcSet && !$srcSet) {
			...
			$node->setAttribute("srcset", $newSrcSetAttribute);
			if($node->hasAttribute('data-srcset') || $node->hasAttribute('data-fastcache-lazyload')) {
				$node->setAttribute("data-srcset", $newSrcSetAttribute);
				list($image, $image_file_size) = $this->fetchImageData($imagePath);
	...
	public function optimize($sOptimizedHtml) {
		...
		if (!File::folderExists(FASTCACHE_CACHE_DIR . 'images')) {
			File::folderCreate(FASTCACHE_CACHE_DIR . 'images');
		}
