...
namespace FastCacheFramework\Mvc;

use FastCacheFramework\Application\Application;
use FastCacheFramework\Container\Container;
use FastCacheFramework\Input\Input;
use FastCacheFramework\Mvc\Engine\EngineInterface;
 ...
 *
 * @package FastCacheFramework\Mvc
 */
class View
{
	/**
	...
	protected $viewEngineMap = array(
		'.tmpl.php' => 'FastCacheFramework\\Mvc\\Engine\\BladeEngine',
		'.php'       => 'FastCacheFramework\\Mvc\\Engine\\PhpEngine',
	);
	...
	protected function getExtension($path)
	{
		...
		return fastcache_array_first($extensions, function($key, $value) use ($path)
		{
			return fastcache_ends_with($path, $value);
		});
	}
	...
	public function renderEach($viewTemplate, $data, $eachItemName, $empty = 'raw|')
	{
		...
		{
			if (fastcache_starts_with($empty, 'raw|'))
			{
				$result = substr($empty, 4);
			}
			elseif (fastcache_starts_with($empty, 'text|'))
			{
	 ...
	 *
	 * @return \FastCacheFramework\Container\Container
	 */
