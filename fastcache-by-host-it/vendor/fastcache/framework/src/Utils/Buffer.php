...
namespace FastCacheFramework\Utils;
...
class Buffer
{
	...
	/**
	 * Should I register the fastcache:// stream wrapper
	 *
	...
	public static function canRegisterWrapper()
	{
		if (is_null(static::$canRegisterWrapper))
		{
			...
			// If Suhosin is installed check if fastcache:// is whitelisted
			if ($hasSuhosin)
			{
				...
				if (!in_array('fastcache://', $whiteList))
				{
					return false;
				}
			}
...
{
	stream_wrapper_register("awf", "\\FastCacheFramework\\Utils\\Buffer");
}
